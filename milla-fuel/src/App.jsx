...
export default function MillaFuel() {
  ...
  const [training, setTraining] = useState("moderate");
  const [trainTime, setTrainTime] = useState("16:00");
  ...
  const S = {
    ...
    stat: { flex:1, background:C.surface, border:`1px solid ${C.border}`, borderRadius:10,
      padding:"8px 6px", textAlign:"center" },
    statVal: { fontSize:16, fontWeight:800 },
    statLbl: { fontSize:9, color:C.muted, marginTop:1 },
    // Status banner
    banner: (col) => ({ background:`${col}15`, border:`1px solid ${col}40`,
      borderRadius:10, padding:"8px 12px", marginTop:10, display:"flex",
    ...
    searchInput: { flex:1, background:C.surface, border:`1px solid ${C.border}`,
      ...
      fontFamily:"inherit" },
    catRow: { display:"flex", gap:6, overflowX:"auto", paddingBottom:8, marginBottom:10,
      scrollbarWidth:"none" },
    ...
    foodItem: { display:"flex", alignItems:"center", justifyContent:"space-between",
    ...
    // Timing selector
    timingRow: { display:"flex", gap:6, overflowX:"auto", paddingBottom:4,
      scrollbarWidth:"none", marginBottom:10 },
    timingBtn: (a) => ({ flexShrink:0, background: a ? C.blueLo : C.surface,
      border:`1px solid ${a ? C.blue : C.border}`, color: a ? C.blue : C.muted,
      borderRadius:20, padding:"5px 12px", fontSize:11, fontWeight: a ? 700 : 500,
      cursor:"pointer", whiteSpace:"nowrap" }),
    // Log
    logItem: { display:"flex", alignItems:"flex-start", gap:10, padding:"9px 0",
      borderBottom:`1px solid ${C.border}` },
    logName: { fontSize:13, color:C.text, flex:1 },
    logMeta: { fontSize:10, color:C.muted, marginTop:2 },
    removeBtn: { background:"none", border:"none", color:C.dim, fontSize:18,
    ...
    trainLabel: (a) => ({ fontSize:14, fontWeight: a ? 700 : 500,
    ...
    // Select
    select: { width:"100%", background:C.surface, border:`1px solid ${C.border}`,
      borderRadius:10, padding:"10px 12px", color:C.text, fontSize:13, outline:"none",
      fontFamily:"inherit", boxSizing:"border-box", marginBottom:8 },
  };
        ...
        <div style={{ padding:"12px 14px 0" }}>
          ...
          <div style={S.card}>
            ...
            <div style={S.statsRow}>
              <div style={S.stat}>
                <div style={{ ...S.statVal, color:C.accent }}>{totals.cal}</div>
                <div style={S.statLbl}>eaten</div>
              </div>
              <div style={S.stat}>
                <div style={S.statVal}>{tgt.cal}</div>
                <div style={S.statLbl}>target</div>
              </div>
              <div style={S.stat}>
                <div style={{ ...S.statVal, color: totals.fluid >= tgt.fluid*0.8 ? C.accent : C.amber }}>
                  {totals.fluid}
                </div>
                <div style={S.statLbl}>ml fluid</div>
              </div>
              <div style={S.stat}>
                <div style={S.statVal}>{tgt.fluid}</div>
                <div style={S.statLbl}>ml target</div>
              </div>
            </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ maxHeight:260, overflowY:"auto", scrollbarWidth:"none" }}>
              ...
              {filtered.length === 0 && (
                <div style={{ padding:"16px 0", textAlign:"center", color:C.muted, fontSize:13 }}>
                  No foods match — add it manually below
                </div>
              )}
            </div>
            ...
            {customOpen && (
              <div style={{ marginTop:10 }}>
                <input style={S.inputField} placeholder="Food name (e.g. Chicken burger)" value={custom.name}
          ...
          {meals.length > 0 && (
            <div style={S.card}>
              ...
              {meals.map(m => (
                <div key={m.id} style={S.logItem}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={S.logMeta}>
                      {TIMING.find(t => t.value === m.timing)?.label || m.timing} · {m.cal}kcal · {m.carbs}g C · {m.protein}g P · {m.fat}g F
                    </div>
                  </div>
              ...
              <button style={{ ...S.ghostBtn, marginTop:10, color:C.red, borderColor:`${C.red}40` }}
                onClick={() => { if(window.confirm("Clear today's log?")) setMeals([]); }}>
                Clear all
              </button>
            </div>
          )}
      ...
      {tab === "training" && (
        <div style={{ padding:"12px 14px 0" }}>
          <div style={S.card}>
            ...
            {TRAINING.map(t => (
              <div key={t.value} style={S.trainOpt(training === t.value)}
                onClick={() => setTraining(t.value)}>
          ...
          <div style={S.card}>
            <div style={S.label}>Timing guide for today</div>
            {[
        ...
        <div style={{ padding:"12px 14px 0" }}>
          <div style={S.card}>
            ...
            <div style={{ marginTop:6 }}>
              <Bar label="Fluid" cur={totals.fluid} max={tgt.fluid} color={C.blue} />
            </div>
          </div>
